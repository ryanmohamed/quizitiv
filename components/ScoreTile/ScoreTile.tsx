    ...
    return (
        <div className={styles.Container}> { quiz && <div className="bg-[var(--bg3)] rounded-md md:h-[240px] md:w-[240px] sm:h-[180px] sm:w-[180px] p-3 relative flex flex-col justify-between" key={index}>
                <div>
                    ...
                    <p className="flex justify-start items-center my-2 gap-1">{[1,2,3,4,5].map((value, key) => (<Star fill={ value <= 3 ? "#257BDF" : "var(--txt4)"} />))}</p>
                </div>
                <p className="self-end md:h-[40px] md:w-[75px] xs:h-[25px] xs:w-[55px] bg-[var(--bg5)] border-[1px] border-[var(--txt5)] rounded-md flex items-center justify-center md:text-2xl sm:text-lg text-[#64B885]">{Math.floor(score/quiz.numQuestions * 100)}% </p>
            </div>
        } </div>
    )
}
